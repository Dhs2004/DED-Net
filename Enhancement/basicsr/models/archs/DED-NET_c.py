...
class IG_MSA(nn.Module):
    def __init__(self, dim, dim_head=64, heads=8, hidden_dim=256, dropout_rate=0.1):
        super().__init__()
        ...
        self.feed_forward = FeedForwardNetwork(dim_head * heads, hidden_dim, dropout_rate)
    ...
    def forward(self, x_in, illu_fea_trans):
        ...
        illu_fea_trans: [b,h,w,c] # illumination feature
        return out: [b,h,w,c]
        """
        b, h, w, c = x_in.shape
        ...
        # 前馈网络
        out_ff = self.feed_forward(out_c)
        
        out_p = self.pos_emb(v_inp.reshape(b, h, w, c).permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        
        out = self.dropout(out_ff + out_p)
        return self.layer_norm1(out_c + out)  # 残差连接和归一化
